(function(A,$){
    ...
    A.module('starter.services', [])

    .value("URI","http://181.49.3.210/DM/bin/fyii/index.php")

    .service('ModuloService', function($q) {

        var Modulos = [
            {"id": "ExplrdorPrdccion", "Name": "Explorador de Producción", "Icon":"icon-book-open icon"},
            {"id": "CnsltaPrdccion", "Name": "Consulta de Producción", "Icon":"icon-page-search icon"},
            {"id": "CnsltaCrrspndncia", "Name": "Consulta de Correspondencia", "Icon":"icon-paper-plane icon"},
            {"id": "Correo", "Name": "Correspondencia", "Icon":"icon-mail pull-left icon"},
        ];
    ...
    .service('GetData', function($q,$http,URI) {
        return {
            findAll: function(dta) {
                var deferred = $q.defer();
                    $http
                        .get(URI, {
                            params: dta
